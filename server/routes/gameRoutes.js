...
function buildSummaryRows(rows) {
  ...
  for (const key of ["household", "city", "country"]) {
    levels[key] = {
      ...
      cost: 50, sustainability: 0, stars: 0, unlocked: 0, streak: 0, best_streak: 0,
    };
  }
  rows.forEach((row) => {
...
// POST /game/reset/:userId/:level — sustainability resets to 0
router.post("/reset/:userId/:level", async (req, res) => {
  try {
    const { userId, level } = req.params;
    await run(
      `UPDATE user_progress SET turns_completed = 0, waste = 50, resources = 50, cost = 50, sustainability = 0, stars = 0, streak = 0
